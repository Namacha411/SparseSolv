...
SparseMatC::SparseMatC(
    const Eigen::SparseMatrix<dcomplex, Eigen::RowMajor> &Mat0) {
  matrix.initialize(Mat0);
}
/* サイズ指定 */
SparseMatC::SparseMatC(slv_int x) { matrix.tempInitialize(x); }
/* 作成済み疎行列データから初期化 */
SparseMatC::SparseMatC(slv_int n_nonZero, const std::vector<slv_int> &rows,
...
//=======================================================*/
/* 同クラスコピー */
SparseMatC::SparseMatC(const SparseMatC &mat) { matrix = mat.matrix; }
/* 同クラスムーブ */
SparseMatC::SparseMatC(SparseMatC &&mat) noexcept {
  matrix = std::move(mat.matrix);
}
/* 行列クラスコピー */
SparseMatC::SparseMatC(const SparseMatBaseC &mat) { matrix = mat; }
/* 行列クラスムーブ */
...
dcomplex *SparseMatC::operator*(const double *vec) const {
  dcomplex *ans;
  SparseMatOperators::mat_vec_product<SparseMatBaseC, dcomplex, double,
                                      dcomplex>(&ans, matrix, vec);
  return ans;
...
dcomplex *SparseMatC::operator*(const dcomplex *vec) const {
  dcomplex *ans;
  SparseMatOperators::mat_vec_product<SparseMatBaseC, dcomplex, dcomplex,
                                      dcomplex>(&ans, matrix, vec);
  return ans;
}
...
std::vector<dcomplex>
SparseMatC::operator*(const std::vector<dcomplex> &vec) const {
  Eigen::VectorXcd eigen_vec =
      Eigen::Map<const Eigen::VectorXcd>(vec.data(), vec.size());
...
SparseMatC SparseMatC::operator*(const SparseMat &mat) const {
  Eigen::SparseMatrix<dcomplex, Eigen::RowMajor> tempMat =
      (matrix.matrix * mat.matrix.matrix).pruned();
...
SparseMatC SparseMatC::operator+(const SparseMat &mat) const {
  ...
  SparseMatOperators::plus_operators<SparseMatBaseC, SparseMatBaseD,
                                     SparseMatBaseC, dcomplex, double>(
      mat_ori, matrix, mat.matrix, 1.0, 1.0, 0, 0);
  /* 結果を本体行列にムーブし、終わる */
  SparseMatC mat_ans(std::move(mat_ori));
  return mat_ans;
}
SparseMatC SparseMatC::operator+(const SparseMatC &mat) const {
  Eigen::SparseMatrix<dcomplex, Eigen::RowMajor> tempMat =
      (matrix.matrix + mat.matrix.matrix);
  SparseMatBaseC mat_ori(std::move(tempMat));
  /* 結果を本体行列にムーブし、終わる */
  SparseMatC mat_ans(std::move(mat_ori));
  return mat_ans;
...
//=======================================================*/
SparseMatC SparseMatC::makeSubMat(slv_int range1a, slv_int range1b,
                                  slv_int range2a, slv_int range2b) {
  SparseMatBaseC mat_ori;
  SparseMatOperators::makeSubMat<SparseMatBaseC, dcomplex>(
      mat_ori, matrix, range1a, range1b, range2a, range2b);
  /* 結果を本体行列にムーブし、終わる */
  SparseMatC mat_ans(std::move(mat_ori));
  return mat_ans;
}
...
//=======================================================*/
void SparseMatC::MatDiv(SparseMatC &matK11, SparseMatC &matK12, slv_int rangeA,
                        slv_int rangeB) {
  SparseMatBaseC mat_ori1;
  SparseMatBaseC mat_ori2;
  SparseMatOperators::MatDiv<SparseMatBaseC, dcomplex>(
      matrix, mat_ori1, mat_ori2, rangeA, rangeB);
  /* 結果を本体行列にムーブし、終わる */
...
SparseMatC SparseMatC::inv() const {
  SparseMatBaseC mat_ori;
  SparseMatOperators::MatInv<SparseMatBaseC, dcomplex, Eigen::MatrixXcd>(
      mat_ori, matrix);
  /* 結果を本体行列にムーブし、終わる */
  SparseMatC mat_ans(std::move(mat_ori));
  return mat_ans;
...
SparseMatC SparseMatC::IC_decomp(dcomplex *diagD, const double accela) const {
  SparseMatBaseC mat_ori;
  SparseMatOperators::IC_decomp<SparseMatBaseC, dcomplex, dcomplex>(
      mat_ori, matrix, diagD, accela);
  /* 結果を本体行列にムーブし、終わる */
  SparseMatC mat_ans(std::move(mat_ori));
  return mat_ans;
}
...
// ● 対角スケーリング
//=======================================================*/
SparseMatC SparseMatC::diagScaling(dcomplex *trans_vec,
                                   const dcomplex *ori_vec) const {
  SparseMatBaseC mat_ori;
