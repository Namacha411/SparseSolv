...
private:
  ...
  std::unique_ptr<std::map<slv_int, DType>[]>
      tempMat; /* 一時保存行列（キーが列位置） */
  Eigen::SparseMatrix<DType, Eigen::RowMajor> matrix; /* Eigenスパース行列 */
  /*-------------------------------------------------------*/
  void add_typeN(slv_int gyo, slv_int retu, DType val) {
...
public:
  ...
  SparseMatTMPL(const std::vector<Eigen::Triplet<DType>>
                    &sparse_data); /* Eigenタプルから初期化 */
  SparseMatTMPL(
      slv_int n_nonZero, const std::vector<slv_int> &rows,
      const std::vector<slv_int> &cols,
      const std::vector<DType> &vals); /* 作成済み疎行列データから初期化 */
  ~SparseMatTMPL();
  ...
  SparseMatTMPL<DType> &
  operator=(const SparseMatTMPL<DType> &Mat); /* 代入オペレータ */
  SparseMatTMPL<DType> &
  operator=(SparseMatTMPL<DType> &&Mat) noexcept; /* 代入オペレータ（ムーブ） */
  ...
  /**/
  void initialize(const Eigen::SparseMatrix<DType, Eigen::RowMajor>
                      &Mat0); /* 確定行列ごと初期化 */
  void initialize(Eigen::SparseMatrix<DType, Eigen::RowMajor>
                      &&Mat0); /* 確定行列ごと初期化 */
  void initialize(const std::vector<Eigen::Triplet<DType>>
                      &sparse_data); /* 確定行列ごと初期化 */
  void initialize(slv_int n_nonZero, const std::vector<slv_int> &rows,
                  const std::vector<slv_int> &cols,
                  const std::vector<DType> &vals); /* 確定行列ごと初期化 */
  /**/
  void fixedInitialize(slv_int size0); /* 空でSparse行列をfix */
  void fix(bool toSquare = false); /* 一時配列を確定させる(bool toSquare:
                                      trueなら0を入れて正方行列にする) */
  void refresh() { fix(); };
  void resetMat(); /* 確定済み行列の値を０に再セット */
  void
  getCols(slv_int *cols_data1,
          slv_int *cols_data2) const; /* 各行の列のスタート・終了位置を返す */
  auto getColPtr() const {
    return matrix.innerIndexPtr();
  }; /* 列のポインタを返す */
  auto getValuePtr() const {
    return matrix.valuePtr();
  }; /* 値のポインタを返す */
  bool isInclude(slv_int gyo, slv_int target_r, slv_int &result_retu)
      const; /* i行目にtarget_r列があるかどうか（あったらそのindexを返す） */
  slv_int getMaxCol() const; /* スパース内の最大の列位置を返す */
  void add(slv_int gyo, slv_int retu, DType val); /* 一時配列にpush */
  void getTargetRowVal(slv_int target, std::vector<slv_int> &row_pos,
                       std::vector<DType> &row_val)
      const; /* 指定した列の非ゼロの行位置と値をvectorに書き出す */
  void getTargetColVal(slv_int target, std::vector<slv_int> &col_pos,
                       std::vector<DType> &col_val)
      const; /* 指定した行の非ゼロの列位置と値をvectorに書き出す */
  void operator*=(const DType xval); /* 自身にスカラ積 */
  void delFlagPosition(
      const bool *flag); /* フラグがTrueの位置の列・行のデータを削除（FEM用） */
  void DiagFlagPosition(
      const bool *
          flag); /* フラグがTrueの位置の列・行のデータをゼロにして対角だけ１に（FEM用）
                  */
  /*---------------------------------------------*/
  /*---------------------------------------------*/
...
// ● コンストラクタ
//=======================================================*/
template <typename DType> SparseMatTMPL<DType>::SparseMatTMPL() {
  size = 0;
  is_fix = false;
}
template <typename DType> SparseMatTMPL<DType>::SparseMatTMPL(slv_int size0) {
  size = size0;
  is_fix = false;
  /* 一時行列初期化 */
  tempInitialize();
...
SparseMatTMPL<DType>::SparseMatTMPL(
    const Eigen::SparseMatrix<DType, Eigen::RowMajor> &Mat0) {
  this->initialize(Mat0);
}
template <typename DType>
void SparseMatTMPL<DType>::initialize(
    const Eigen::SparseMatrix<DType, Eigen::RowMajor> &Mat0) {
  size = Mat0.rows();
  is_fix = true;
  matrix = Mat0;
...
template <typename DType>
SparseMatTMPL<DType>::SparseMatTMPL(
    Eigen::SparseMatrix<DType, Eigen::RowMajor> &&Mat0) {
  this->initialize(Mat0);
}
template <typename DType>
void SparseMatTMPL<DType>::initialize(
...
/* 作成済み疎行列データから初期化 */
template <typename DType>
SparseMatTMPL<DType>::SparseMatTMPL(slv_int n_nonZero,
                                    const std::vector<slv_int> &rows,
                                    const std::vector<slv_int> &cols,
                                    const std::vector<DType> &vals) {
  this->initialize(n_nonZero, rows, cols, vals);
}
template <typename DType>
void SparseMatTMPL<DType>::initialize(slv_int n_nonZero,
                                      const std::vector<slv_int> &rows,
                                      const std::vector<slv_int> &cols,
                                      const std::vector<DType> &vals) {
  is_fix = true;
...
template <typename DType>
SparseMatTMPL<DType>::SparseMatTMPL(
    const std::vector<Eigen::Triplet<DType>> &sparse_data) {
...
template <typename DType>
SparseMatTMPL<DType> &
SparseMatTMPL<DType>::operator=(const SparseMatTMPL<DType> &Mat) {
  /* 既存データは削除 */
  if (tempMat) {
    tempMat.reset();
  }
...
template <typename DType>
SparseMatTMPL<DType> &
SparseMatTMPL<DType>::operator=(SparseMatTMPL<DType> &&Mat) noexcept {
  /* 既存データは削除 */
  if (tempMat) {
    tempMat.reset();
  }
...
// ● 一時行列の初期化
//=======================================================*/
template <typename DType> void SparseMatTMPL<DType>::tempInitialize() {
...
void SparseMatTMPL<DType>::resizeInitialize(slv_int new_size) {
  /* スパース位置が確定していたら、エラー*/
  if (is_fix) {
    std::cout << " this initialize method is not for fixed matrix!"
              << std::endl;
    exit(1);
  }
  /*  一時行列があったら削除 */
...
// ● 一時配列を確定させる
//=======================================================*/
template <typename DType> void SparseMatTMPL<DType>::fix(bool toSquare) {
  ...
  /* toSquare=true
   * なら、行数の方が大きいなら正方、列の方が大きいなら長方形サイズにする */
...
// ● 確定済み行列の値を０に再セット
//=======================================================*/
template <typename DType> void SparseMatTMPL<DType>::resetMat() {
  /* 確定済みじゃなければ何もしない */
  if (!is_fix) {
    return;
  }
...
//=======================================================*/
template <typename DType>
void SparseMatTMPL<DType>::getCols(slv_int *cols_data1,
                                   slv_int *cols_data2) const {
...
//=======================================================*/
template <typename DType> bool SparseMatTMPL<DType>::isEmpty() const {
...
//=======================================================*/
template <typename DType>
bool SparseMatTMPL<DType>::isInclude(slv_int gyo, slv_int target_r,
                                     slv_int &result_retu) const {
  if (is_fix) {
    slv_int count = 0;
    /* ポインタをたどって探す */
...
//=======================================================*/
template <typename DType>
void SparseMatTMPL<DType>::getTargetRowVal(slv_int target,
                                           std::vector<slv_int> &row_pos,
                                           std::vector<DType> &row_val) const {
  if (!is_fix) {
    return;
  }
  row_pos.clear();
...
//=======================================================*/
template <typename DType>
void SparseMatTMPL<DType>::getTargetColVal(slv_int target,
                                           std::vector<slv_int> &col_pos,
                                           std::vector<DType> &col_val) const {
  if (!is_fix) {
    return;
  }
  col_pos.clear();
...
// ● スパース内の最大の列位置を返す
//=======================================================*/
template <typename DType> slv_int SparseMatTMPL<DType>::getMaxCol() const {
  /* 確定前のとき */
  if (!is_fix) {
    slv_int max = 0;
    for (slv_int i = 0; i < size; i++) {
...
void SparseMatTMPL<DType>::printMat(const std::string &str) {
  ...
  const slv_int total_size = matrix.nonZeros();
  for(slv_int i = 0 ; i <total_size; i++){
          std::cout << row_ptr[i] << ", " << col_ptr[i] << ", "<< val_ptr[i] <<
  std::endl;
  }*/
