...
/* サイズ指定 */
SparseMat::SparseMat(slv_int x) { matrix.tempInitialize(x); }
/* 作成済み疎行列データから初期化 */
SparseMat::SparseMat(slv_int n_nonZero, const std::vector<slv_int> &rows,
...
//=======================================================*/
/* 同クラスコピー */
SparseMat::SparseMat(const SparseMat &mat) { matrix = mat.matrix; }
/* 同クラスムーブ */
SparseMat::SparseMat(SparseMat &&mat) noexcept {
  matrix = std::move(mat.matrix);
}
/* 行列クラスコピー */
SparseMat::SparseMat(const SparseMatBaseD &mat) { matrix = mat; }
/* 行列クラスムーブ */
SparseMat::SparseMat(SparseMatBaseD &&mat) noexcept { matrix = std::move(mat); }
...
double *SparseMat::operator*(const double *vec) const {
  double *ans;
  SparseMatOperators::mat_vec_product<SparseMatBaseD, double, double, double>(
      &ans, matrix, vec);
  return ans;
}
dcomplex *SparseMat::operator*(const dcomplex *vec) const {
  dcomplex *ans;
  SparseMatOperators::mat_vec_product<SparseMatBaseD, double, dcomplex,
...
std::vector<dcomplex>
SparseMat::operator*(const std::vector<dcomplex> &vec) const {
...
SparseMatC SparseMat::operator+(const SparseMatC &mat) const {
  ...
  SparseMatOperators::plus_operators<SparseMatBaseC, SparseMatBaseD,
      ...
      mat_ori, mat.matrix, matrix, 1.0, 1.0, 0, 0);
  /* 結果を本体行列にムーブし、終わる */
  SparseMatC mat_ans(std::move(mat_ori));
  return mat_ans;
}
...
//=======================================================*/
SparseMat SparseMat::makeSubMat(slv_int range1a, slv_int range1b,
                                slv_int range2a, slv_int range2b) {
  SparseMatBaseD mat_ori;
  SparseMatOperators::makeSubMat<SparseMatBaseD, double>(
...
//=======================================================*/
void SparseMat::MatDiv(SparseMat &matK11, SparseMat &matK12, slv_int rangeA,
                       slv_int rangeB) {
  SparseMatBaseD mat_ori1;
  SparseMatBaseD mat_ori2;
  SparseMatOperators::MatDiv<SparseMatBaseD, double>(matrix, mat_ori1, mat_ori2,
                                                     rangeA, rangeB);
  /* 結果を本体行列にムーブし、終わる */
...
SparseMat SparseMat::inv() const {
  SparseMatBaseD mat_ori;
  SparseMatOperators::MatInv<SparseMatBaseD, double, Eigen::MatrixXd>(mat_ori,
                                                                      matrix);
  /* 結果を本体行列にムーブし、終わる */
  SparseMat mat_ans(std::move(mat_ori));
  return mat_ans;
...
SparseMat SparseMat::IC_decomp(double *diagD, const double accela) const {
  SparseMatBaseD mat_ori;
  SparseMatOperators::IC_decomp<SparseMatBaseD, double, double>(mat_ori, matrix,
                                                                diagD, accela);
  /* 結果を本体行列にムーブし、終わる */
  SparseMat mat_ans(std::move(mat_ori));
  return mat_ans;
...
// ● 対角スケーリング
//=======================================================*/
SparseMat SparseMat::diagScaling(double *trans_vec,
                                 const double *ori_vec) const {
  SparseMatBaseD mat_ori;
