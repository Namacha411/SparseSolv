...
//=======================================================*/
bool MatSolvers::solveICMRTR(const slv_int size0, const double conv_cri,
                             const int max_ite, const double accera,
                             const SparseMat &matA, const double *vecB,
                             double *results, bool init) {
  ...
  norm = sqrt(norm);
  return solveICMRTR(size, conv_cri, max_ite, accera, norm, matA, vecB, results,
                     init);
}
...
bool MatSolvers::solveICMRTR(const slv_int size0, const double conv_cri,
                             ...
                             std::vector<double> &results, bool init) {
  ...
  norm = sqrt(norm);
  bool bl = solveICMRTR(size0, conv_cri, max_ite, accera, norm, matA, vecBa,
                        results_a, init);
  for (slv_int i = 0; i < size0; i++) {
...
bool MatSolvers::solveICMRTR(const slv_int size0, const double conv_cri,
                             ...
                             const double *vecB, double *results, bool init) {
  /* 対角スケーリングあり */
  if (is_diag_scale) {
    return solveICMRTR_diag(size0, conv_cri, max_ite, accera, normB, matA, vecB,
                            results, init);
  }
  ...
  bool bl = solveICMRTR(size0, conv_cri, max_ite, normB, diagD, matA.matrix,
                        matL.matrix, matL_tr.matrix, vecB, results, init);
  delete[] diagD;
  return bl;
}
...
// ● MRTRで解く・外部実行本体
//=======================================================*/
bool MatSolvers::solveICMRTR_diag(const slv_int size0, const double conv_cri,
                                  const int max_ite, const double accera,
                                  const double normB, const SparseMat &matA,
                                  const double *vecB, double *results,
                                  bool init) {
  /* 対角スケーリング */
  ...
  /* 解く */
  bool bl = solveICMRTR(size0, conv_cri, max_ite, normB2, diagD, matDAD.matrix,
                        matL.matrix, matL_tr.matrix, vecB2, results, init);
...
bool MatSolvers::solveICMRTR(const slv_int size0, const double conv_cri,
  ...
  normB2 = sqrt(normB2);
  return solveICMRTR(size, conv_cri, max_ite, accera, normB2, matA, vecB,
                     results, init);
}
...
bool MatSolvers::solveICMRTR(const slv_int size0, const double conv_cri,
                             ...
                             std::vector<dcomplex> &results, bool init) {
  ...
  double normB2 = abs(norm);
  normB2 = sqrt(normB2);
  bool bl = solveICMRTR(size0, conv_cri, max_ite, accera, normB2, matA, vecBa,
                        results_a, init);
  for (slv_int i = 0; i < size0; i++) {
    results[i] = results_a[i];
  }
...
// ● MRTRで解く（本体）
//=======================================================*/
bool MatSolvers::solveICMRTR(const slv_int size, const double conv_cri,
                             const int max_ite, const double normB,
                             const double *diagD, const SparseMatBaseD &matA,
                             const SparseMatBaseD &matL,
...
bool MatSolvers::solveICMRTR(const slv_int size, const double conv_cri,
                             const int max_ite, const double normB,
                             const dcomplex *diagD, const SparseMatBaseC &matA,
                             const SparseMatBaseC &matL,
                             const SparseMatBaseC &matL_tr,
                             const dcomplex *vecB, dcomplex *results,
                             bool init) {
