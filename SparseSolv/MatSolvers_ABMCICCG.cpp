...
/*
    ...
    This is a parallel version of the ICCG solver using the Algebraic Block
   Multi-Color (ABMC) ordering. The solver is based on the following paper:
    "Algebraic Block Multi-Color Ordering Method for Parallel Multi-Threaded
   Sparse Triangular Solver in ICCG Method" proposed by T. Iwashita, H.
   Nakashima, and Y. Takahashi, in the Conference: Parallel & Distributed
   Processing Symposium (IPDPS), 2012 IEEE 26th International.
*/
...
//
void MatSolvers::sortAlgebraicMultiColor(
    slv_int size0, const SparseMatBaseD &A, SparseMatBaseD &PA, const double *b,
    double *Pb, std::vector<std::vector<slv_int>> &color_list,
    std::vector<slv_int> &ordering, std::vector<slv_int> &reverse_ordering,
    int num_colors) {
...
//
void MatSolvers::makeAlgebraicBlock(
    slv_int size0, const SparseMatBaseD &A, SparseMatBaseD &Mb,
      ...
      if (block_assign[j] == -1) {
        ...
        while (block_list[i].size() < num_blocks && que.size() != 0) {
          if (block_assign[que.front()] == -1) {
            ...
            for (slv_int k = start_pos[que.front()]; k < end_pos[que.front()];
                 k++) {
              if (block_assign[col_ptr[k]] == -1) {
                que.push(col_ptr[k]);
              }
...
//
void MatSolvers::sortAlgebraicBlockMultiColor(
    slv_int size0, const SparseMatBaseD &A, SparseMatBaseD &PA, const double *b,
    double *Pb, std::vector<std::vector<slv_int>> &block_list,
  ...
  sortAlgebraicMultiColor(max_block, Mb, PMb, y, z, color_list, block_ordering,
                          reverse_block_ordering, num_colors);
...
bool MatSolvers::parallelIccgSolv(slv_int size0, const SparseMatBaseD &A,
                                  const SparseMatBaseD &L,
                                  const SparseMatBaseD &Lt,
                                  std::vector<std::vector<slv_int>> &block_list,
                                  std::vector<std::vector<slv_int>> &color_list,
                                  const double *diagD, const double *b,
                                  double *x, double eps, int numItr) {
...
/* *********************************************************** */
void MatSolvers::parallelIcSolv(slv_int size0, const SparseMatBaseD &L,
                                const SparseMatBaseD &Lt,
                                std::vector<std::vector<slv_int>> &block_list,
                                std::vector<std::vector<slv_int>> &color_list,
...
#pragma omp parallel for private(bl, l, mu, t, idx)
    for (k = 0; k < color_list[col].size(); k++) {
      ...
      for (l = 0; l < block_list[bl].size(); l++) {
        idx = block_list[bl][l];
        t = b(idx);
        for (mu = start_pos[idx]; mu < end_pos[idx] - 1;
             mu++) { // numNZ-1 is the diagnal !!
    ...
    for (k = color_list[col].size() - 1; k >= 0; k--) {
      ...
      for (l = block_list[bl].size() - 1; l >= 0; l--) {
        ...
        for (nu = start_pos2[idx] + 1; nu < end_pos2[idx]; nu++) {
          // t -= 1.0 / val_ptr2[start_pos2[idx]] * val_ptr2[nu] *
          // x(col_ptr2[nu]);
...
//
//
bool MatSolvers::solveICCGwithABMC(const slv_int size0, const double conv_cri,
                                   const int max_ite, const double accera,
                                   const SparseMat &matA, const double *vec_b,
                                   double *vec_x, int num_blocks,
                                   int num_colors, bool init) {
  ...
  norm = sqrt(norm);
  // call
  bool bl = solveICCGwithABMC(size0, conv_cri, max_ite, accera, norm, matA,
                              vec_b, vec_x, num_blocks, num_colors);
  return bl;
}
...
bool MatSolvers::solveICCGwithABMC(const slv_int size0, const double conv_cri,
                                   ...
                                   const double *vec_b, double *vec_x,
                                   int num_blocks, int num_colors, bool init) {
  bool is_conv = false;
  ...
  sortAlgebraicBlockMultiColor(n, matA.matrix, PA.matrix, vec_b, vec_Pb,
                               block_list, color_list, ordering,
                               reverse_ordering, num_blocks, num_colors);
  PA.fix();
  ...
  is_conv =
      parallelIccgSolv(n, PA.matrix, matL.matrix, matL_tr.matrix, block_list,
                       color_list, diagD, vec_Pb, vec_x, epsilon, max_ite);
...
bool MatSolvers::solveICCGwithABMC(const slv_int size0, const double conv_cri,
  ...
  norm = sqrt(norm);
  bool bl = solveICCGwithABMC(size0, conv_cri, max_ite, accera, norm, matA,
                              vecBa, results_a, num_blocks, num_colors, init);
  for (int i = 0; i < size0; i++) {
    vec_x[i] = results_a[i];
  }
  delete[] vecBa;
