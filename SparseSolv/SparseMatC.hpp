...
public:
  ...
  SparseMatC(
      ...
      const std::vector<dcomplex> &vals); /* 作成済み疎行列データから初期化 */
  ...
  bool isInclude(slv_int gyo, slv_int target_r, slv_int &result_retu) const {
    return matrix.isInclude(gyo, target_r, result_retu);
  }; /* i行目にtarget_r列があるかどうか（あったらそのindexを返す） */
  void add(slv_int gyo, slv_int retu, dcomplex val) {
    matrix.add(gyo, retu, val);
  }; /* 一時配列にpush */
  void getTargetRowVal(slv_int target, std::vector<slv_int> &row_pos,
                       std::vector<dcomplex> &row_val) const {
    matrix.getTargetRowVal(target, row_pos, row_val);
  }; /* 指定した列の非ゼロの行位置と値をvectorに書き出す */
  void getTargetColVal(slv_int target, std::vector<slv_int> &col_pos,
                       std::vector<dcomplex> &col_val) const {
    matrix.getTargetColVal(target, col_pos, col_val);
  }; /* 指定した行の非ゼロの列位置と値をvectorに書き出す */
  slv_int getMaxCol() const {
    return matrix.getMaxCol();
  }; /* スパース内の最大の列位置を返す */
  void delFlagPosition(const bool *flag) {
    matrix.delFlagPosition(flag);
  }; /* フラグがTrueの位置の列・行のデータを削除（FEM用） */
  void DiagFlagPosition(const bool *flag) {
    matrix.DiagFlagPosition(flag);
  ...
  SparseMatC makeSubMat(
      slv_int range1a, slv_int range1b, slv_int range2a,
      slv_int range2b); /* 範囲[r1a, r1b]×[r2a,
                           r2b]の部分を抜き出して部分行列を作り、Bに渡す */
  void MatDiv(SparseMatC &matK11, SparseMatC &matK12, slv_int rangeA,
              slv_int rangeB); /* 列rangeBを境に、行列を2つに分ける。行rangeAより下は削除する
                                */
  SparseMatC getMatLower() const; /* 下三角行列の取得 */
  SparseMatC getMatUpper() const; /* 上三角行列の取得 */
  SparseMatC
  inv() const; /* 逆行列（密アルゴリズム・大行列でやるとメモリが吹き飛ぶ！） */
  SparseMatC
  makePrsdInv(double eps) const; /* 疑似逆行列用の行列（A^T*A+epsI）を作成 */
  /* 不完全コレスキー分解 */
