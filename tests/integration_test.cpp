...
TEST_F(IntegrationTest, FiniteElementLikeSystem) {
    ...
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 5; ++j) {
            ...
            // Neighboring points
            if (j > 0) mat->add(idx, idx - 1, -1.0);  // Left
            if (j < 4) mat->add(idx, idx + 1, -1.0);  // Right
            if (i > 0) mat->add(idx, idx - 5, -1.0);  // Up
            if (i < 4) mat->add(idx, idx + 5, -1.0);  // Down
        }
    }
    mat->fix();
    ...
    std::cout << "ICCG solve time: " << duration.count() << " microseconds" << std::endl;
...
TEST_F(IntegrationTest, ElectromagneticLikeComplexSystem) {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    std::cout << "Complex ICCG solve time: " << duration.count() << " microseconds" << std::endl;
...
TEST_F(IntegrationTest, SolverComparison) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (i > 0) mat->add(i, i-1, -0.8);
        if (i < n-1) mat->add(i, i+1, -0.8);
        if (i > 1) mat->add(i, i-2, -0.2);
        if (i < n-2) mat->add(i, i+2, -0.2);
    }
    mat->fix();
    ...
    // Test IC-MRTR
    {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        bool converged = solver->solveICMRTR(n, 1e-8, 100, 1.0, *mat, rhs, solution);
        auto end = std::chrono::high_resolution_clock::now();
...
TEST_F(IntegrationTest, PerformanceScaling) {
    ...
    for (int n : sizes) {
        ...
        for (int i = 0; i < n; ++i) {
            mat->add(i, i, 4.0);
            if (i > 0) mat->add(i, i-1, -1.0);
            if (i < n-1) mat->add(i, i+1, -1.0);
        }
        mat->fix();
