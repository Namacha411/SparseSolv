...
TEST_F(SparseMatTest, LargeMatrix) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i > 0) mat->add(i, i-1, -1.0);  // Lower diagonal
        if (i < n-1) mat->add(i, i+1, -1.0);  // Upper diagonal
        if (i > 1) mat->add(i, i-2, -0.5);  // Second lower diagonal
        if (i < n-2) mat->add(i, i+2, -0.5);  // Second upper diagonal
    }
