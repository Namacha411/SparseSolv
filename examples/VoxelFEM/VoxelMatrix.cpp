...
void VoxelMesh::make_matrix(SRLfem::SparseMat &matA) {
  ...
  /* 磁気抵抗率テンソル（v[0]:Vxx, v[1]:Vxy, v[2]:Vxz, v[3]:Vyy, v[4]:Vyz,
   * v[5]:Vzz） */
  double v_tensor[9];
  ...
  for (int i = 0; i < element_num; i++) {
    ...
    for (int k = 0; k < 12; k++) {
      k_id = Edge_ids[k];
      /* ゼロの既知辺なら、さようなら */
      if (EdgeData[k_id].isZeroEdge())
        continue;
      for (int m = k; m < 12; m++) {
        m_id = Edge_ids[m];
        /* ゼロの既知辺なら、さようなら */
        if (EdgeData[m_id].isZeroEdge())
          continue;
        /* 係数セット */
