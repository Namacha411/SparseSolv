...
public:
  ...
  double getMinX() {
    return NodeData[nodes[0]].getPosX();
  }; /* 要素の最小X位置返し */
  double getMinY() {
    return NodeData[nodes[0]].getPosY();
  }; /* 要素の最小Y位置返し */
  double getMinZ() {
    return NodeData[nodes[0]].getPosZ();
  }; /* 要素の最小Z位置返し */
  double getMaxX() {
    return NodeData[nodes[6]].getPosX();
  }; /* 要素の最大X位置返し */
  double getMaxY() {
    return NodeData[nodes[6]].getPosY();
  }; /* 要素の最大Y位置返し */
  double getMaxZ() {
    return NodeData[nodes[6]].getPosZ();
  }; /* 要素の最大Z位置返し */
};
