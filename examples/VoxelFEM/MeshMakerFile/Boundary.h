...
private:
  ...
  static void Cholesky(int *clmn_size, int **column, double **matrixA,
                       int *clmnL_size, int **columnL, double **matrixL,
                       int *clmL_tr_size, int **columnL_tr, double **matrixL_tr,
                       double *diagD); /* 不完全コレスキー分解！！ */
  static void preProcess(int *clmnL_size, int **columnL, double **matrixL,
                         int *clmnL_tr_size, int **columnL_tr,
                         double **matrixL_tr, double *diagD, double *vecR,
                         double *vec); /* 前処理 */
  static double inner_products(const double *a, const double *b); /* 内積計算 */
public:
  static void setSize(int s) { size = s; }; /* サイズセット */
  static void ICCGsolve(int s, int *clmn_size, int **column, double **matrixA,
                        double *vecB, double *results); /* ICCGで解く */
  static void ICCGsolve(int *clmn_size, int **column, double **matrixA,
                        double *vecB, double *results); /* ICCGで解くver2 */
};
int ICCG::size;
...
/*//=======================================================
  // ● ICCGで解く
  //=======================================================*/
void ICCG::ICCGsolve(int s, int *clmn_size, int **column, double **matrixA,
                     double *vecB, double *results) {
  size = s;
...
/*//=======================================================
  // ● ICCGで解く
  //=======================================================*/
void ICCG::ICCGsolve(int *clmn_size, int **column, double **matrixA,
                     double *vecB, double *results) {
...
#pragma omp parallel for
  ...
  /* コレスキー分解 */
  Cholesky(clmn_size, column, matrixA, clmnL_size, columnL, matrixL,
           clmnL_tr_size, columnL_tr, matrixL_tr, diagD);
  /* 前処理 */
  preProcess(clmnL_size, columnL, matrixL, clmnL_tr_size, columnL_tr,
             matrixL_tr, diagD, vecB, results);
  double normB = 0;
...
/*//=======================================================
  // ● 不完全コレスキー分解！！
  //=======================================================*/
void ICCG::Cholesky(int *clmn_size, int **column, double **matrixA,
                    int *clmnL_size, int **columnL, double **matrixL,
                    int *clmL_tr_size, int **columnL_tr, double **matrixL_tr,
                    double *diagD) {
  double s;
  ...
  for (int i = 0; i < size; i++) {
    ...
    for (int jj = 0; jj < c_size; jj++) {
      ...
      J_count[j]++;
      //				cout << "Mat L is " << i << ", " << jj
      //<< ", " << matrixL[i][jj] << ", " << matrixA[i][jj] <<endl; 				if(isnan(
      //matrixL[i][jj] ) || isinf(matrixL[i][jj])) exit(1); }else{ 	cout <<
      //"!?!?!?!?!?!?! ==> "<< i << ", " << columnL[i][jj] << " & " <<
      //matrixL[i][jj] << endl;
      //}
      A_ptr++;
                  ...
                  for (int i = 0; i < j_size; i++) {
                          ...
                          for (int jj = 0; jj < c_size; jj++) {
                                  if(columnL[the_i][jj] == j){
                                          ...
                                          cout << "LLL is " << j << ", " <<
     the_i << " & " << the_i << ", " << columnL[the_i][jj] << " --> " << temp1
     << ", " << temp2 << endl; if(fabs(temp1-temp2) > 1.0e-10){ exit(1);
                                          }
                                  }
                          }
                  }
...
/*//=======================================================
  // ● 前処理
  //=======================================================*/
void ICCG::preProcess(int *clmnL_size, int **columnL, double **matrixL,
                      int *clmnL_tr_size, int **columnL_tr, double **matrixL_tr,
                      double *diagD, double *vecR, double *vec) {
  double s;
  double *vec_ptr = vec;
  /* 第一方程式計算 */
  for (int i = 0; i < size; i++) {
    s = vecR[i]; //*vecR;//
    const int c_size =
        clmnL_size[i] - 1; //(*clmnL_size) - 1;//clmnL_size[i] - 1;
    double *L_ptr = matrixL[i];
    ;                        //*matrixL;//matrixL[i];
