...
private:
   ...
   */
public:
  MeshMaker(int xn, int yn, int zn, double *xyz_L1, double *xyz_L2,
            int *change_xyz); /* コンストラクタ */
  ~MeshMaker();
...
/*//=======================================================
  // ● コンストラクタ
  //=======================================================*/
MeshMaker::MeshMaker(int xn, int yn, int zn, double *xyz_L1, double *xyz_L2,
                     int *change_xyz) {
  /* 各種サイズ設定 */
  x_length1 = xyz_L1[0];
  y_length1 = xyz_L1[1];
  ...
  edge_num = (x_num * (y_num + 1) + (x_num + 1) * y_num) * (z_num + 1) +
             z_num * (x_num + 1) * (y_num + 1);
...
void MeshMaker::setNodeNum(SearchTree *S_tree) {
  ...
  for (int i = 0; i < node_num; i++) {
    ...
    tempPtr->setID(i);
    vx = (temp_x > change_x
              ? x_length1 * change_x + (temp_x - change_x) * x_length2
              : temp_x * x_length1);
    vy = (temp_y > change_y
              ? y_length1 * change_y + (temp_y - change_y) * y_length2
              : temp_y * y_length1);
    vz = (temp_z > change_z
              ? z_length1 * change_z + (temp_z - change_z) * z_length2
              : temp_z * z_length1);
...
/*//=======================================================
  // ● 辺番号設定用サブルーチン
  //=======================================================*/
void MeshMaker::setEdgeSubRutin(int i, SearchTree *S_tree, int dir,
                                int &count) {
  int id1, id2;
...
void MeshMaker::setElementSubRtin(int x, int y, int z, SearchTree *S_tree) {
  ...
  int es[] = {edge12, edge8, edge1,  edge6, edge11, edge4,
              edge2,  edge9, edge10, edge7, edge3,  edge5};
  /* 要素に代入 */
...
void MeshMaker::WriteVTK() {
  ...
  for (int i = 0; i < element_num; i++) {
    ...
    fp << "8 " << inode[0] << " " << inode[1] << " " << inode[2] << " "
       << inode[3] << " " << inode[4] << " " << inode[5] << " " << inode[6]
       << " " << inode[7] << endl;
  }
  /* 要素タイプの指定 */
