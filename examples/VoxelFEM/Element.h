...
public:
  ...
  double getMinX() {
    return NodeData[nodes[0]].getX();
  }; /* 要素の最小X位置返し */
  double getMinY() {
    return NodeData[nodes[0]].getY();
  }; /* 要素の最小Y位置返し */
  double getMinZ() {
    return NodeData[nodes[0]].getZ();
  }; /* 要素の最小Z位置返し */
  double getMaxX() {
    return NodeData[nodes[6]].getX();
  }; /* 要素の最大X位置返し */
  double getMaxY() {
    return NodeData[nodes[6]].getY();
  }; /* 要素の最大Y位置返し */
  double getMaxZ() {
    return NodeData[nodes[6]].getZ();
  }; /* 要素の最大Z位置返し */
  static void setNodeData(Node *n) { NodeData = n; };
  static void setEdgeData(Edge *n) { EdgeData = n; };
  void setMaterial(int x, double perm) {
    mat_type = x;
    permeability = perm;
  };                                     /* 要素の材質指定 */
